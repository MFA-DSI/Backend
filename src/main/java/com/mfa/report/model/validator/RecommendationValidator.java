...
public class RecommendationValidator implements Consumer<Recommendation> {
  ...
  public void accept(Recommendation recommendation, String committerId) {
    ...
    if (!recommendation.getActivity().getMission().getDirection().getResponsible().contains(user)
        && recommendation.isApproved()) {
      throw new BadRequestException(
          "Une recommandation doit être approver par une responsabe de la direction pour être afficher");
    }
  }
